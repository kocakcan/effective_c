...
* Alignment
*
* Object types have alignment requirements that place restrictions on the addresses at
* which objects of that type may be allocated. An alignment represents the number of b
* ytes between successive addresses at which a given object can be allocated. CPUs may
* have different behaviour when accessing aligned data (for example, the data address 
* is a multiple of the data size) versus unaligned data.
* 	Some machine instructions can perform multiple accesses on non-word boundaries
* , but there may be a performance penalty. A word is a natural, fixed-size unit of a 
* data handled by the instruction set or the hardware of the processor. Some platforms
* cannot access unaligned memory. Alignment requirements may depend on the CPU word si
* ze (typically, 16, 32, or 64 bits).
* 	Generally, C programmers need not concern themselves with alignment requiremen
* ts, because the compiler chooses suitable alignments for its various types. Dynamica
* lly allocated memory from malloc is required to be sufficiently aligned for all stan
* dard types, including arrays and structures. However, on rare occasions, you might n
* eed to override the compiler's default choices; for example, to align data on the bo
* undaries of the memory cache lines that must start at power-of-two address boundarie
* s, or to meet other system-specific requirements. Traditionally, these requirements 
* were met by linker commands, or by overallocating memory with malloc followed by rou
* nding the user address upward, or similar operations involving other nonstandard fac
* ilties.
* 	C11 introduced a simple, forward-compatible mechanism for specifying alignment
* s. Alignments are represented as values of type size_t. Every valid alignment value 
* is a nonnegative integral power of two. An object type imposes a default alignment 
* requirement on every object of that type: a stricter alignment (a larger power of tw
* o) can be requested using the alignment specifier (_Alignas). You can include an ali
* gnment specifier in the declaration specifiers of a declaration. Listing 2-7 uses th
* e alignment specifier to ensure that good_buff is properly aligned (bad_buff) may ha
* ve incorrect alignment for member-access expressions).
*
* 	struct S {
*		int i;
*		double d;
...
*		struct S *good_s_ptr = (struct S *)good_buff; // correct pointer alig.
*	}
*	Listing 2-7: Use of the _Alignas keyword
* 	Alignments are ordered from weaker to stronger (also called stricter) alignmen
* ts. Stricter alignments have larger alignment values. And address that satisfies an 
